...
const IS_PREACT = true;
const IS_REACT = false;
...
export default class Autocomplete extends Component {
  static defaultProps = {
    ...
    tAssistiveHint: () => '. Use @ symbol to auto-select tools. When autocomplete results are available use up and down arrows to review and enter to select.  Touch device users, explore by touch or with swipe gestures.',
  ...
  handleInputChange (event) {
    ...
    let searchForOptions = showAllValues || (!queryEmpty && queryChanged && queryLongEnough)
    

    // get currently selected word
    const getCurrentWord = (text, pos) => {

      // Find the word’s start by moving backwards until whitespace or BOF
      let start = pos;
      while (start > 0 && !/\s/.test(text[start - 1])) {
        start--;
      }

      // Find the word’s end by moving forwards until whitespace or EOF
      let end = pos;
    ...
    query = getCurrentWord(query, event.target.selectionStart);
    if (query.charAt(0) !== '@') {
  ...
  render () {
    ...
    const ariaProps = {
      'aria-describedby': `prompt-hint ${ariaHint ? assistiveHintID : null}`,
    ...
    let textarea = this.elementReferences[-1];
    console.log(query, textarea?.selectionStart);
    // TO DO: move getSelectedWord and use start and end to replace with query
